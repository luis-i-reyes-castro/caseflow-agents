...
class StateMachineBase(ABC) :
    ...
    def build_dummy_methods_for_on_enter_and_on_exit(self) -> None :
        """
        For each state's 'on_enter' callback function (say 'callback'):
        If 'self.callback' does not exist then assign it a dummy function
        taking no arguments and returning nothing.
        Same for each state's 'on_exit' callback function.
        """
    ...
    def get_actions(self) -> set[str] :
        ...
        Returns:
            Set of callback names as strings
        """
